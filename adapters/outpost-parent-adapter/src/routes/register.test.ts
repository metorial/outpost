...
import { registerHandler, type RegisterHandlerDeps } from './register';
...
let mountApp = (deps: RegisterHandlerDeps) => {
  let app = new Hono();
  app.onError((error, c) => {
  ...
  app.post('/register', registerHandler(deps));
  return app;
};

let baseDeps = (overrides: Partial<RegisterHandlerDeps> = {}): RegisterHandlerDeps => ({
...
describe('outpost-parent-adapter registerHandler', () => {
  it("signs the forwarded request as this outpost's own service and preserves the body", async () => {
    ...
    let res = await app.request('/register', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body
    });
    ...
    expect(url).toBe('https://parent.example.com/outpost/register');
  ...
  describe('a registration already relayed by a nested Outpost', () => {
    let setup = async () => {
    ...
    let signedRegisterRequest = async (opts: {
      ...
      let url = new URL('http://localhost/register');
    ...
    it('reuses the incoming proxy_context instead of re-deriving one from its own connection', async () => {
      ...
      let req = await signedRegisterRequest({
        instancePrivateKey,
        instanceToken,
    ...
    it('rejects a forwarded registration whose Outpost signature does not verify', async () => {
      let { tokens, instancePrivateKey, instanceToken } = await setup();
      let { fetch, calls } = buildFetchMock();
      let app = mountApp(baseDeps({ fetch, tokens }));
